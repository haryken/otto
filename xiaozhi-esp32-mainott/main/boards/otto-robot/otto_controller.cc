...
private:
    ...
    bool motors_enabled_ = false;  // Trạng thái động cơ
    int last_explore_action_ = -1;  // Lưu hành động trước đó để tránh lặp lại
    ...
    static void ActionTask(void* arg) {
        ...
        // Bật động cơ khi bắt đầu task
        if (!controller->motors_enabled_) {
            controller->otto_.AttachServos();
            controller->motors_enabled_ = true;
            ESP_LOGI(TAG, "Động cơ đã được bật (ActionTask)");
        }
        ...
        while (true) {
            if (xQueueReceive(controller->action_queue_, &params, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                /* Không gọi DetachServos ở đây: ledc_stop đồng loạt làm servo analog mất lệnh cùng lúc
                 * → thường thấy vài con cùng quay một hướng rồi mới “tắt”. Giữ PWM để giữ tư thế. */
    ...
    static void ExploreTask(void* arg) {
        ...
        // Bật động cơ khi bắt đầu chế độ khám phá
        if (!controller->motors_enabled_) {
            controller->otto_.AttachServos();
            controller->motors_enabled_ = true;
            ESP_LOGI(TAG, "Động cơ đã được bật (ExploreTask)");
        }
        
        ESP_LOGI(TAG, "探索模式启动 - 开始随机探索");
        ...
        while (controller->is_explore_mode_active_) {
            ...
            // 随机步数/次数 (tối thiểu 4, tối đa 8 bước)
            int steps = 4 + (r % 5);  // 4-8步
            int speed = 1000 + (r % 300);  // 1000-1300
            int amount = 20 + (r % 20);   // 20-40度
            
            // 随机选择动作类型 (0-6)
            // 0: 前进, 1: 后退, 2: 左转, 3: 右转, 4: Swing, 5: Moonwalk, 6: 暂停
            int action_type = r % 7;
            
            // Tránh lặp lại hành động trước đó - nếu trùng thì chọn lại
            int retry_count = 0;
            ...
            switch (action_type) {
                case 0:  // 前进
                    ESP_LOGI(TAG, "探索: 前进 %d 步", steps);
                    controller->otto_.Walk(steps, speed, 1, 30);
                    break;
                case 1:  // 后退
                    ESP_LOGI(TAG, "探索: 后退 %d 步", steps);
                    controller->otto_.Walk(steps, speed, -1, 30);
                    break;
                case 2:  // 左转
                    ESP_LOGI(TAG, "探索: 左转 %d 步", steps);
                    controller->otto_.Turn(steps, speed, 1, 30);
                    break;
                case 3:  // 右转
                    ESP_LOGI(TAG, "探索: 右转 %d 步", steps);
                    controller->otto_.Turn(steps, speed, -1, 30);
                    break;
                case 4:  // Swing (左右摇摆)
                    ESP_LOGI(TAG, "探索: 摇摆 %d 次", steps);
                    controller->otto_.Swing(steps, speed, amount);
                    break;
                case 5:  // Moonwalk (太空步)
                    ESP_LOGI(TAG, "探索: 太空步 %d 步", steps);
                    {
                        int moonwalk_dir = (r & 0x1) ? 1 : -1;  // 随机方向
                ...
                default:
                    ESP_LOGI(TAG, "探索: 暂停");
                    // 不调用Home() khi pause, chỉdelay
                    vTaskDelay(pdMS_TO_TICKS(300 + (r % 700)));  // 0.3-1秒
                    controller->is_action_in_progress_ = false;
                    // 动作之间的延迟
                    vTaskDelay(pdMS_TO_TICKS(300 + (esp_random() % 400)));  // 0.3-0.7秒
                    continue;  // 跳过下面的Home()调用
            }
            
            // 每个动作后回到home位置 (chỉ khi không phải pause)
            controller->otto_.Home(false);
            controller->is_action_in_progress_ = false;
            
            // 动作之间的延迟，让动作更自然，同时yield cho các task khác
            int delay_ms = 400 + (esp_random() % 600);  // 0.4-1秒随机延迟
        ...
        controller->is_action_in_progress_ = false;
        controller->last_explore_action_ = -1;  // Reset
        
        ESP_LOGI(TAG, "探索模式已停止");
    ...
    void StartExploreMode(int speed) {
        ...
        if (!motors_enabled_) {
            ...
            ESP_LOGI(TAG, "Động cơ đã được bật (StartExploreMode)");
        }
        ...
        last_explore_action_ = -1;  // Reset hành động trước đó
        
        // 创建探索任务 (priority thấp hơn để không block các task khác)
        if (explore_task_handle_ == nullptr) {
            xTaskCreate(ExploreTask, "otto_explore", 1024 * 4, this, 
                       5, &explore_task_handle_);  // Priority 5 (thấp hơn action task)
        }
        
        ESP_LOGI(TAG, "探索模式已启动");
    }
    ...
    void StopExploreMode() {
        ...
        // 等待任务结束 (tăng thời gian chờ để action hoàn thành)
        if (explore_task_handle_ != nullptr) {
            // 给任务一点时间完成当前动作
            for (int i = 0; i < 10; i++) {  // 等待最多1秒
                if (explore_task_handle_ == nullptr) {
                    break;  // Task đã tự kết thúc
                }
    ...
    void QueueAction(int action_type, int steps, int speed, int direction, int amount) {
        ...
        // Bật động cơ khi có hành động mới (SELF CONTROL, đứng, hoặc bất kỳ hành động nào)
        if (!motors_enabled_) {
            otto_.AttachServos();
            motors_enabled_ = true;
            ESP_LOGI(TAG, "Động cơ đã được bật (QueueAction - SELF CONTROL)");
        }
...
public:
    OttoController() {
        ...
        // Bật động cơ khi khởi động
        motors_enabled_ = true;
        ESP_LOGI(TAG, "Động cơ đã được bật khi khởi động");
    ...
    void RegisterMcpTools() {
        ...
        // 探索模式：连续随机探索，直到调用stop
        mcp_server.AddTool(
            "self.otto.explore",
            "探索模式：让Otto连续随机探索，包括前进、后退、左转、右转、摇摆等动作，"
            "模拟人类自由走动。模式将持续运行直到调用self.otto.stop停止。"
            "speed: 基础动作速度(500-1500，数值越小越快，可选，默认800-1200随机)",
            PropertyList({Property("speed", kPropertyTypeInteger, 1000, 500, 1500)}),
            [this](const PropertyList& properties) -> ReturnValue {
                int speed = properties["speed"].value<int>();
                StartExploreMode(speed);
                return "探索模式已启动，机器人将开始随机探索。调用self.otto.stop可停止。";
            });
        ...
        // 系统工具
        mcp_server.AddTool("self.otto.stop", "立即停止所有动作，包括探索模式", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               // 停止探索模式
                               StopExploreMode();
