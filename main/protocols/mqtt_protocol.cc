        ...
        if (strcmp(type->valuestring, "hello") == 0) {
            ...
            if (session_id == nullptr || session_id_ == session_id->valuestring) {
                ...
                Application::GetInstance().Schedule([this, alive]() {
                    if (*alive) {
                        Application::GetInstance().PrepareAudioChannelClose();
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        if (sequence < remote_sequence_) {
            // Out-of-order/duplicate UDP packets are expected occasionally; don't spam warnings.
            ESP_LOGD(TAG, "Received audio packet with old sequence: %lu, expected: %lu", sequence, remote_sequence_);
            return;
        }
        if (sequence != remote_sequence_ + 1) {
            const uint32_t expected = remote_sequence_ + 1;
            const uint32_t gap = (sequence > expected) ? (sequence - expected) : 0;
            // Small gaps (1-2 frames) are handled by jitter/PLC; keep logs quiet.
            if (gap > 3) {
                ESP_LOGW(TAG, "Received audio packet with wrong sequence: %lu, expected: %lu (gap=%lu)",
                         sequence, expected, gap);
            } else {
                ESP_LOGD(TAG, "Received audio packet with wrong sequence: %lu, expected: %lu (gap=%lu)",
                         sequence, expected, gap);
            }
        }
