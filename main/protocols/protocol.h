...
struct AudioStreamPacket {
    ...
    // For UDP audio stream ordering (MqttProtocol UDP transport).
    // 0 means "unknown / not sequenced" (e.g. websocket audio, local OGG sounds).
    uint32_t sequence = 0;
    float playback_gain = 1.0f;  // PCM gain for local UI sounds (e.g. popup beep)
    bool frame_loss = false;     // UDP gap: play silence instead of decoding payload
