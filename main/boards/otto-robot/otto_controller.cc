...
#define OTTO_NVS_NAMESPACE "otto"
#define OTTO_NVS_KEY_MOTOR_TEST_FWD "motor_fwd"

static void SaveMotorTestForwardToNvs(bool enable) {
    nvs_handle_t nvs;
    if (nvs_open(OTTO_NVS_NAMESPACE, NVS_READWRITE, &nvs) != ESP_OK) {
        ESP_LOGW(TAG, "Failed to open NVS for motor test save");
        return;
    }
    const uint8_t value = enable ? 1 : 0;
    esp_err_t err = nvs_set_u8(nvs, OTTO_NVS_KEY_MOTOR_TEST_FWD, value);
    if (err == ESP_OK) {
        err = nvs_commit(nvs);
    }
    nvs_close(nvs);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Saved motor test forward to NVS: %d", enable ? 1 : 0);
    } else {
        ESP_LOGW(TAG, "Failed to save motor test forward to NVS: %s", esp_err_to_name(err));
    }
}

static bool LoadMotorTestForwardFromNvs() {
    nvs_handle_t nvs;
    if (nvs_open(OTTO_NVS_NAMESPACE, NVS_READONLY, &nvs) != ESP_OK) {
        return false;
    }
    uint8_t value = 0;
    const esp_err_t err = nvs_get_u8(nvs, OTTO_NVS_KEY_MOTOR_TEST_FWD, &value);
    nvs_close(nvs);
...
private:
    ...
    TaskHandle_t explore_task_handle_ = nullptr;
    TaskHandle_t motor_test_forward_task_handle_ = nullptr;
    ...
    static void ExploreTask(void* arg) {
        OttoController* controller = static_cast<OttoController*>(arg);
        controller->otto_.AttachServos();

        ESP_LOGI(TAG, "Chế độ khám phá: bắt đầu di chuyển ngẫu nhiên");
        vTaskDelay(pdMS_TO_TICKS(500));

        while (controller->is_explore_mode_active_) {
            if (!controller->is_explore_mode_active_) {
                break;
            }

            uint32_t r = esp_random();
            int steps = 4 + (r % 5);
            int speed = 1000 + (r % 300);
            int amount = 20 + (r % 20);
            int action_type = r % 7;

            int retry_count = 0;
            while (action_type == controller->last_explore_action_ && retry_count < 10) {
                r = esp_random();
                action_type = r % 7;
                retry_count++;
            }
            controller->last_explore_action_ = action_type;
            ...
            switch (action_type) {
                case 0:
                    ESP_LOGI(TAG, "Khám phá: tiến %d bước", steps);
                    controller->otto_.Walk(steps, speed, 1, 30);
                    break;
                case 1:
                    ESP_LOGI(TAG, "Khám phá: lùi %d bước", steps);
                    controller->otto_.Walk(steps, speed, -1, 30);
                    break;
                case 2:
                    ESP_LOGI(TAG, "Khám phá: rẽ trái %d bước", steps);
                    controller->otto_.Turn(steps, speed, 1, 30);
                    break;
                case 3:
                    ESP_LOGI(TAG, "Khám phá: rẽ phải %d bước", steps);
                    controller->otto_.Turn(steps, speed, -1, 30);
                    break;
                case 4:
                    ESP_LOGI(TAG, "Khám phá: lắc %d lần", steps);
                    controller->otto_.Swing(steps, speed, amount);
                    break;
                case 5:
                    ESP_LOGI(TAG, "Khám phá: moonwalk %d bước", steps);
                    {
                        int moonwalk_dir = (r & 0x1) ? 1 : -1;
                        controller->otto_.Moonwalker(steps, speed, amount, moonwalk_dir);
                    }
                    break;
                case 6:
                default:
                    ESP_LOGI(TAG, "Khám phá: tạm dừng");
                    vTaskDelay(pdMS_TO_TICKS(300 + (r % 700)));
                    controller->is_action_in_progress_ = false;
        ...
        controller->otto_.Home(true);
        controller->is_action_in_progress_ = false;
        controller->last_explore_action_ = -1;
        ESP_LOGI(TAG, "Chế độ khám phá đã dừng");
        controller->explore_task_handle_ = nullptr;
        vTaskDelete(nullptr);
    }

    // Same walk path as self control: QueueAction(ACTION_WALK) -> ActionTask -> otto_.Walk().
    static constexpr int kMotorTestWalkSteps = 3;
    static constexpr int kMotorTestWalkSpeed = 700;
    static constexpr int kMotorTestWalkDirection = 1;
    static constexpr int kMotorTestWalkArmSwing = 50;

    static void MotorTestForwardTask(void* arg) {
        auto* controller = static_cast<OttoController*>(arg);
        controller->otto_.AttachServos();
        ESP_LOGI(TAG, "Wifi motor test: walk forward loop started");

        while (controller->motor_test_forward_active_.load()) {
            PowerManager::PauseBatteryUpdate();
            controller->is_action_in_progress_ = true;
            // Same gait as self.otto.action walk (steps/speed/direction/arm_swing).
            controller->otto_.Walk(kMotorTestWalkSteps, kMotorTestWalkSpeed, kMotorTestWalkDirection,
                                   kMotorTestWalkArmSwing);
            controller->otto_.Home(false);
            controller->is_action_in_progress_ = false;
            PowerManager::ResumeBatteryUpdate();
        ...
        controller->otto_.Home(true);
        controller->is_action_in_progress_ = false;
        ESP_LOGI(TAG, "Wifi motor test: walk forward loop stopped");
        controller->motor_test_forward_task_handle_ = nullptr;
        vTaskDelete(nullptr);
    }

    void SetMotorTestForwardEnabled(bool enable) {
        SaveMotorTestForwardToNvs(enable);

        if (enable) {
            if (motor_test_forward_active_.load()) {
                return;
            }
            StopExploreMode();
            xQueueReset(action_queue_);
            otto_.AttachServos();
            motor_test_forward_active_.store(true);
            if (motor_test_forward_task_handle_ == nullptr) {
                xTaskCreate(MotorTestForwardTask, "otto_wifi_walk", 4096, this, 5,
                            &motor_test_forward_task_handle_);
            }
            ESP_LOGI(TAG, "Wifi motor test: enabled");
        } else {
            if (!motor_test_forward_active_.load()) {
                return;
            }
            motor_test_forward_active_.store(false);
            ...
            ESP_LOGI(TAG, "Wifi motor test: disabled");
        }
    }

    void StartExploreMode(int /*speed*/) {
        if (is_explore_mode_active_) {
            ESP_LOGW(TAG, "Chế độ khám phá đang chạy");
            return;
        }
        if (motor_test_forward_active_.load()) {
            ESP_LOGW(TAG, "Motor test forward is active, cannot start explore mode");
            return;
        }
        ...
        is_explore_mode_active_ = true;
        last_explore_action_ = -1;

        if (explore_task_handle_ == nullptr) {
            xTaskCreate(ExploreTask, "otto_explore", 1024 * 4, this, 5, &explore_task_handle_);
        }
        ESP_LOGI(TAG, "Chế độ khám phá đã bật");
    }

    void StopExploreMode() {
        if (!is_explore_mode_active_) {
            return;
        }

        is_explore_mode_active_ = false;
        ...
        if (explore_task_handle_ != nullptr) {
            ...
            if (explore_task_handle_ != nullptr) {
                vTaskDelete(explore_task_handle_);
                explore_task_handle_ = nullptr;
            }
        }
        ...
        PowerManager::ResumeBatteryUpdate();
        ESP_LOGI(TAG, "Chế độ khám phá đã tắt");
    }
...
public:
    ...
    void RegisterMcpTools() {
        ...
        mcp_server.AddTool(
            "self.otto.explore",
            "Chế độ khám phá: Otto di chuyển ngẫu nhiên (tiến, lùi, rẽ, lắc, moonwalk, tạm dừng) "
            "cho đến khi gọi self.otto.stop. speed: tốc độ cơ bản (500-1500, nhỏ hơn = nhanh hơn).",
            PropertyList({Property("speed", kPropertyTypeInteger, 1000, 500, 1500)}),
            [this](const PropertyList& properties) -> ReturnValue {
                int speed = properties["speed"].value<int>();
                StartExploreMode(speed);
                return "Chế độ khám phá đã bật. Gọi self.otto.stop để dừng.";
            });

        mcp_server.AddTool("self.otto.stop",
                           "Dừng ngay mọi hành động (kể cả chế độ khám phá) và về tư thế đứng",
                           PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               SetMotorTestForwardEnabled(false);
                               StopExploreMode();
                               if (action_task_handle_ != nullptr) {
        ...
        mcp_server.AddTool("self.otto.get_status",
                           "Trạng thái robot: moving (đang động/chế độ khám phá) hoặc idle",
                           PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
                               return (is_action_in_progress_ || is_explore_mode_active_) ? "moving"
                                                                                          : "idle";
                           });
        ...
        mcp_server.AddTool(
            ...
            "Dùng khi người dùng muốn nghe một bài (tên bài / ca sĩ). Trả về id, title khi bắt đầu phát.",
        ...
        mcp_server.AddTool(
            ...
            "Kết thúc hội thoại hiện tại và bật chế độ cấu hình WiFi: robot phát hotspot (AP) để người dùng đổi mạng, "
            "giống lúc máy chưa kết nối WiFi lần đầu. Trang cấu hình có tab Wi-Fi và chọn cấp độ khóa học. "
            "**LƯU Ý** Chỉ gọi sau khi đã hỏi người dùng xác nhận.",
            PropertyList(),
            [](const PropertyList& properties) -> ReturnValue {
                auto& board = static_cast<WifiBoard&>(Board::GetInstance());
                board.EnterWifiConfigMode();
                return "Đang vào chế độ cấu hình WiFi. Hãy kết nối hotspot của robot trên điện thoại hoặc máy tính, "
                       "mở trình duyệt theo hướng dẫn trên màn hình để đổi WiFi.";
            });
...
void InitializeOttoController(const HardwareConfig& hw_config) {
    if (g_otto_controller == nullptr) {
        Application::GetInstance().GetAudioService().SetStopLocalPlaybackHandler([]() {
            OttoMusic::Stop();
        });
        g_otto_controller = new OttoController(hw_config);

        // WiFi config portal calls this; run on main thread (same as MCP/self control).
        WifiManager::GetInstance().SetMotorTestForwardCallback([](bool enable) {
            Application::GetInstance().Schedule([enable]() { OttoWifiConfigMotorTestForward(enable); });
        });

        if (LoadMotorTestForwardFromNvs()) {
            ESP_LOGI(TAG, "Restoring motor test forward from NVS");
            xTaskCreate(
                [](void* arg) {
                    (void)arg;
                    vTaskDelay(pdMS_TO_TICKS(2500));
                    Application::GetInstance().Schedule([]() { OttoWifiConfigMotorTestForward(true); });
                    vTaskDelete(nullptr);
                },
                "otto_mtest_restore", 2048, nullptr, 3, nullptr);
        }
...
void OttoWifiConfigMotorTestForward(bool enable) {
    if (g_otto_controller == nullptr) {
        ESP_LOGW(TAG, "OttoWifiConfigMotorTestForward: controller not ready");
        return;
    }
