...
| self.system.reconfigure_wifi | 进入WiFi配网模式 | 断开当前会话，开启热点供手机/电脑改WiFi（需用户确认后再调用） |
...
### Mắt xanh (GIF đã chỉnh)

Các file trong `components/txp666__otto-emoji-gif-component/gifs/` đã được chỉnh: mắt **vuông bo góc màu xanh**, mỗi biểu cảm một kiểu. Script: `scripts/otto_gif_blue_eyes.py`. Bản gốc 240×240: `gifs/_backup_original/`.

GIF nhẹ cho ESP32 (vẫn chạy khi TTS, không pause):

```bash
python scripts/otto_gif_blue_eyes.py --light
```

(~120×120, ~½ frame, palette 64; firmware scale 2× lên màn 240. Tùy chọn: `--size 96` nhẹ hơn nữa.)

Khôi phục GIF gốc: copy từ `_backup_original` vào `gifs/`, rồi `idf.py build flash`.

Sau khi sửa GIF phải **build + flash** (partition `assets` chứa GIF). Không chỉ flash app — nếu vẫn mắt trắng, chạy `idf.py build flash` hoặc xóa `build/generated_assets.bin` rồi build lại. Thư mục `gifs/_backup_original` không được đóng gói (đã sửa script build).
