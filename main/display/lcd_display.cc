...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    if (image->IsGif()) {
        ...
        if (gif_controller_->IsLoaded()) {
            ...
            lv_image_set_src(emoji_image_, gif_controller_->image_dsc());
            if (gif_controller_->width() > 0 && gif_controller_->width() < width_) {
                // Light Otto GIFs are 120x120; scale 2x to fill the LCD without 240² decode cost.
                lv_image_set_scale(emoji_image_, 256 * width_ / gif_controller_->width());
            } else {
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    // Set parent text color and screen background
    lv_obj_set_style_text_font(screen, text_font, 0);
    lv_obj_set_style_text_color(screen, lvgl_theme->text_color(), 0);
    lv_obj_set_style_bg_color(screen, lvgl_theme->background_color(), 0);
