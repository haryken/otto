...
/* PCM chunks waiting for I2S. Too few → underruns / “lag giật” when Wi-Fi jitters; 60ms/chunk @ OPUS_FRAME_DURATION_MS */
...
// UDP TTS jitter buffer (sequence-ordered) to hide 1-2 packet losses.
// 0: off (old behavior)
// 1: on  (buffer a few packets; fill gaps with silence)
#ifndef CONFIG_AUDIO_JITTER_BUFFER_ENABLE
#define CONFIG_AUDIO_JITTER_BUFFER_ENABLE 1
#endif

#ifndef AUDIO_JITTER_PREFILL_PACKETS
#define AUDIO_JITTER_PREFILL_PACKETS 6
#endif

#ifndef AUDIO_JITTER_MAX_BUFFER_PACKETS
#define AUDIO_JITTER_MAX_BUFFER_PACKETS 48
#endif
...
public:
    ...
    bool IsIdle();
    /** True while TTS/downlink packets are queued or playing (mic should stay muted). */
    bool IsDownlinkBusy();
    ...
    /** When true, mic/AFE capture stays off (e.g. local MP3 playback). Blocks enable calls. */
    void SetCaptureSuspended(bool suspended);
    bool IsCaptureSuspended() const { return capture_suspended_.load(); }

    /** When true, drop server TTS/UDP audio so local music owns the playback queue. */
    void SetLocalPlaybackActive(bool active);
    bool IsLocalPlaybackActive() const { return local_playback_active_.load(); }

    /** Board may register (e.g. Otto MP3) to stop local playback on wake word / open mic. */
    void SetStopLocalPlaybackHandler(std::function<void()> handler);
    void StopLocalPlaybackIfActive();

    /** Re-enable mic/I2S timers after MP3 (power manager may have shut input off). */
    void RestoreAudioHardwareAfterLocalPlayback();
    ...
    /** Drop pending mic encode/send buffers (e.g. before injecting a fixed uplink clip). */
    void ClearUplinkQueues();
    /** Demux OGG and send each packet via callback (bypasses send queue size limit). Returns {sent, total}. */
    std::pair<size_t, size_t> SendOggUplink(
        const std::string_view& ogg,
        const std::function<bool(std::unique_ptr<AudioStreamPacket>)>& send_fn);
    /** Short UI chime directly to playback queue (survives ResetDecoder in EnableVoiceProcessing). */
    void PlayNotificationSound(const std::string_view& sound, float playback_gain = 1.0f);
    /** Queue PCM for I2S (e.g. local MP3). Blocks until queue has space. */
    void EnqueuePlaybackPcm(std::vector<int16_t>&& pcm);
...
private:
    ...
    // UDP audio (sequence>0) is stored here and emitted in-order.
    std::map<uint32_t, std::unique_ptr<AudioStreamPacket>> jitter_buffer_;
    uint32_t jitter_next_sequence_ = 0;
#endif
    ...
    std::atomic<bool> capture_suspended_{false};
    std::atomic<bool> local_playback_active_{false};
    std::function<void()> stop_local_playback_handler_;
