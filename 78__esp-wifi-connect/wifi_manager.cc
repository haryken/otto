...
void WifiManager::StartConfigAp() {
    ...
    if (motor_test_forward_callback_) {
        config_ap_->OnMotorTestForward(motor_test_forward_callback_);
    }
...
void WifiManager::SetMotorTestForwardCallback(std::function<void(bool)> callback) {
    std::lock_guard<std::mutex> lock(mutex_);
    motor_test_forward_callback_ = std::move(callback);
    if (config_ap_) {
        config_ap_->OnMotorTestForward(motor_test_forward_callback_);
    }
}
