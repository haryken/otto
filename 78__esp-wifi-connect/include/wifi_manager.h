...
public:
    ...
    /** Register Otto walk-forward motor test handler (invoked from config web UI). */
    void SetMotorTestForwardCallback(std::function<void(bool)> callback);
...
private:
    ...
    std::function<void(bool)> motor_test_forward_callback_;
