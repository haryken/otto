...
public:
    ...
    /** Otto motor walk test from course config tab (enable = walk forward loop). */
    void OnMotorTestForward(std::function<void(bool)> callback);
    void InvokeMotorTestForward(bool enable);
...
private:
    ...
    std::function<void(bool)> on_motor_test_forward_;
