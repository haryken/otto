...
/* NVS key max 15 chars. 0 = chip/custom, 1..5 = presets, 6 = manual custom_mac. */
#define NVS_KEY_PRESET_MAC_IDX "preset_mac"
#define NVS_KEY_CUSTOM_MAC "custom_mac"
#define NVS_KEY_PRESET_IDENTITY "preset_identity"  /* legacy bool */
#define DEFAULT_PRESET_MAC_IDX 1  /* explorers */
...
static void LoadCustomMacFromNvs(nvs_handle_t nvs, std::string& out)
{
    out.clear();
    char buf[CUSTOM_MAC_STR_MAX] = {0};
    size_t len = sizeof(buf);
    if (nvs_get_str(nvs, NVS_KEY_CUSTOM_MAC, buf, &len) == ESP_OK && buf[0] != '\0') {
        out = buf;
    }
}

static uint8_t LoadPresetMacIdxFromNvs(nvs_handle_t nvs)
{
    int32_t idx32 = 0;
    uint8_t idx = 0;
    bool found = false;
    if (nvs_get_i32(nvs, NVS_KEY_PRESET_MAC_IDX, &idx32) == ESP_OK) {
        ...
        uint8_t legacy = 0;
        if (nvs_get_u8(nvs, NVS_KEY_PRESET_IDENTITY, &legacy) == ESP_OK && legacy != 0) {
...
static esp_err_t SavePresetMacIdxToNvs(nvs_handle_t nvs, uint8_t idx)
{
    esp_err_t err = nvs_set_i32(nvs, NVS_KEY_PRESET_MAC_IDX, static_cast<int32_t>(idx));
    if (err == ESP_OK) {
        nvs_erase_key(nvs, NVS_KEY_PRESET_IDENTITY);
    }
    return err;
}
...
WifiConfigurationAp::WifiConfigurationAp()
{
    ...
    preset_mac_idx_ = DEFAULT_PRESET_MAC_IDX;
}
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    if (err == ESP_OK) {
        ...
        preset_mac_idx_ = LoadPresetMacIdxFromNvs(nvs);
        LoadCustomMacFromNvs(nvs, custom_mac_);
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Re-sync from NVS so UI matches flash (in-memory can be stale across requests)
            nvs_handle_t nvs_cfg;
            if (nvs_open("wifi", NVS_READONLY, &nvs_cfg) == ESP_OK) {
                this_->preset_mac_idx_ = LoadPresetMacIdxFromNvs(nvs_cfg);
                LoadCustomMacFromNvs(nvs_cfg, this_->custom_mac_);
                nvs_close(nvs_cfg);
            }
            ...
            cJSON_AddNumberToObject(json, "preset_mac_idx", this_->preset_mac_idx_);
            if (!this_->custom_mac_.empty()) {
                cJSON_AddStringToObject(json, "custom_mac", this_->custom_mac_.c_str());
            }

            {
                uint8_t motor_test_fwd = 0;
                nvs_handle_t nvs_otto;
                if (nvs_open("otto", NVS_READONLY, &nvs_otto) == ESP_OK) {
                    nvs_get_u8(nvs_otto, "motor_fwd", &motor_test_fwd);
                    nvs_close(nvs_otto);
                }
                cJSON_AddBoolToObject(json, "motor_test_forward", motor_test_fwd != 0);
            }
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            } else if (cJSON_IsNumber(remember_bssid)) {
                this_->remember_bssid_ = (remember_bssid->valuedouble != 0.0);
                err = nvs_set_u8(nvs, "remember_bssid", this_->remember_bssid_ ? 1 : 0);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save remember_bssid: %d", err);
                }
            ...
            } else if (cJSON_IsNumber(sleep_mode)) {
                this_->sleep_mode_ = (sleep_mode->valuedouble != 0.0);
                err = nvs_set_u8(nvs, "sleep_mode", this_->sleep_mode_ ? 1 : 0);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save sleep_mode: %d", err);
                }
            ...
            if (cJSON_IsNumber(preset_mac_idx)) {
                ...
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save " NVS_KEY_PRESET_MAC_IDX ": %d", err);
                } else {
                    ESP_LOGI(TAG, "Saved preset_mac_idx=%d", this_->preset_mac_idx_);
                }
            }

            cJSON *custom_mac = cJSON_GetObjectItem(json, "custom_mac");
            if (cJSON_IsString(custom_mac) && custom_mac->valuestring) {
                const char* mac_str = custom_mac->valuestring;
                if (mac_str[0] == '\0') {
                    this_->custom_mac_.clear();
                    nvs_erase_key(nvs, NVS_KEY_CUSTOM_MAC);
                } else if (IsValidMacString(mac_str)) {
                    this_->custom_mac_ = mac_str;
                    err = nvs_set_str(nvs, NVS_KEY_CUSTOM_MAC, this_->custom_mac_.c_str());
                    if (err != ESP_OK) {
                        ESP_LOGE(TAG, "Failed to save " NVS_KEY_CUSTOM_MAC ": %d", err);
                    } else {
                        ESP_LOGI(TAG, "Saved custom_mac=%s", this_->custom_mac_.c_str());
                    }
                } else {
                    ESP_LOGW(TAG, "Invalid custom_mac ignored: %s", mac_str);
                }
            }
            ...
            if (cJSON_IsNumber(max_tx_power)) {
                ...
                err = esp_wifi_set_max_tx_power(pwr);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to set WiFi power: %d (other settings still saved)", err);
                } else {
                    this_->max_tx_power_ = static_cast<int8_t>(pwr);
            ...
            ESP_LOGI(TAG, "Saved settings: ota_url=%s, max_tx_power=%d, remember_bssid=%d, sleep_mode=%d, preset_mac_idx=%d",
                this_->ota_url_.c_str(), this_->max_tx_power_, this_->remember_bssid_, this_->sleep_mode_, this_->preset_mac_idx_);
            return ESP_OK;
        },
        .user_ctx = this
    };
    ...
    // Robot: motor test forward toggle (for board bring-up / servo testing).
    httpd_uri_t motor_test_forward = {
        .uri = "/robot/motor_test_forward",
        .method = HTTP_POST,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            char *buf = nullptr;
            size_t buf_len = req->content_len;
            ...
            const cJSON *enable_item = cJSON_GetObjectItemCaseSensitive(json, "enable");
            const bool enable = cJSON_IsTrue(enable_item) || (cJSON_IsNumber(enable_item) && enable_item->valuedouble != 0.0);
            cJSON_Delete(json);

            ESP_LOGI(TAG, "Motor test forward: enable=%d", enable ? 1 : 0);
            auto* ap = static_cast<WifiConfigurationAp*>(req->user_ctx);
            if (ap != nullptr) {
                ap->InvokeMotorTestForward(enable);
            }
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &motor_test_forward));
...
void WifiConfigurationAp::OnMotorTestForward(std::function<void(bool)> callback)
{
    on_motor_test_forward_ = std::move(callback);
}

void WifiConfigurationAp::InvokeMotorTestForward(bool enable)
{
    if (on_motor_test_forward_) {
        on_motor_test_forward_(enable);
    } else {
        ESP_LOGW(TAG, "Motor test forward: callback not registered");
    }
}
